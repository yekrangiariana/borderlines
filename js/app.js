...
// Cache for backdrop SVG paths to avoid recalculating projections
const backdropPathCache = new Map();
...
function createBackdropToken(country) {
  ...
  // Check cache first
  let pathD = backdropPathCache.get(countryCode);
  if (!pathD) {
    ...
    pathD = path(feature);
    if (pathD && countryCode) {
      backdropPathCache.set(countryCode, pathD);
    }
  }
...
function renderOutlineBackdrop(data) {
  ...
  const isUltraWide = window.matchMedia("(min-width: 2200px)").matches;
  const viewportHeight = Math.max(window.innerHeight || 0, 320);

  // Larger tokens on big screens means fewer rows needed
  const tokenHeight = isMobile ? 50 : isUltraWide ? 185 : isWide ? 114 : 72;
  const lineGap = isMobile ? 12 : isWide ? 24 : 16;
  ...
  // Fewer rows on large screens - bigger tokens fill the space
  const rows = isUltraWide
    ? Math.min(5, computedRows)
    : isWide
      ? Math.min(6, computedRows)
      : Math.max(4, Math.min(10, computedRows));
  const occupiedHeight = tokenHeight + (rows - 1) * rowStep;
  const startTop = Math.max(
    0,
    Math.floor((viewportHeight - occupiedHeight) / 2),
  );

  // Fewer tokens per row on large screens (bigger tokens)
  const perRow = isMobile ? 8 : isUltraWide ? 6 : isWide ? 7 : 11;
